...
def main():
    ...
    joint_angles = [
        ...
        [-0.9609192089764935, -1.6092656004622086,
         2.083684802209344, -0.4737098360681413,
         2.1798764814624683, 0.0010164451199345947],
        [-2.117058728768466, -2.03062812314778,
         2.3830978312053315, -0.3514397131160969,
         1.0250250419482896, -0.0005404554449164323],
        [0.9605936424587247, -1.5259875995406373,
         -2.0752874172898848, -2.6816817943523397,
         -2.1812980830328312, 0.0008248485472286404]]
    ...
    #         print i + 1
    #         ur5_1.move_group.set_pose_target(p)
    ...
    for c in range(9, 12):
        ...
        for pack in range(12):
            ...
            package = shelf.get(pack)
            x, y = package.position
            tja = joint_angles[pack]

            # set min time
            t_min1 = rospy.rostime.Duration(0, read_dur('{}{}'.format(x, y)))
            t_min2 = rospy.rostime.Duration(0, read_dur('{}{}'.format(x, y)))
            print t_min1, t_min2

            play_traj(ur5_1, 'ur5_1_default_{}{}'.format(x_, y_))
            ur5_1.move_group.stop()
            rospy.sleep(1)
            ...
            for i in range(rounds):
                ...
                ur5_1.move_group.set_joint_value_target(tja)
                for _ in range(plan_attempts):
                    plan1 = ur5_1.move_group.plan()
                    t1 = rospy.Time.now()
                    f1 = ur5_1.move_group.execute(plan1, wait=True)
                ...
                # while not play_traj(ur5_1, 'ur5_1_{}{}_default'.format(x, y)):
                #     pass
                # ur5_1.move_group.stop()
                #
                # rospy.sleep(1)
                #
                # while not play_traj(ur5_1, 'ur5_1_default_{}{}'.format(x, y)):
                #     pass
                # ur5_1.move_group.stop()
                ...
                dt1 = t2 - t1
                # noinspection PyUnboundLocalVariable
                if dt1 < t_min1 and f1:
                    t_min1 = dt1
                    # noinspection PyUnboundLocalVariable
                    plan1min = plan1

                    # noinspection PyUnboundLocalVariable
                    save_traj(plan1min, 'ur5_1_{}{}_{}{}'.format(x_, y_, x, y))
                    # save_txt(str(t_min1), '{}{}'.format(x, y))

                rospy.sleep(1)

                ur5_1.move_group.set_joint_value_target(dja)
                for _ in range(plan_attempts):
                    plan2 = ur5_1.move_group.plan()
                    t3 = rospy.Time.now()
                    f2 = ur5_1.move_group.execute(plan2, wait=True)
                ...
                dt2 = t4 - t3
                # noinspection PyUnboundLocalVariable
                if dt2 < t_min2 and f2:
                    t_min2 = dt2
                    # noinspection PyUnboundLocalVariable
                    plan2min = plan2

                    # noinspection PyUnboundLocalVariable
                    save_traj(plan2min, 'ur5_1_{}{}_{}{}'.format(x, y, x_, y_))
                    # save_txt(str(t_min2), '{}{}'.format(x, y))

                rospy.sleep(1)

                # play_traj(ur5_1, 'ur5_1_{}{}_default'.format(x_, y_))
                # ur5_1.move_group.stop()
                ...
                print '**********************************************************************'
                print c, pack
                print 'current:\t', dt1, dt2
                print 'best:\t\t', t_min1, t_min2
                print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'

            append_to_file('costs.txt', '{}\t{}\t{}\n'.format(c, pack, t_min1))
            append_to_file('costs.txt', '{}\t{}\t{}\n'.format(pack, c, t_min2))
